...
@section('title', 'Provision Account')
    ...
    <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 p-10 border border-white fade-in">
        <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <select name="role" id="role" required class="w-full bg-slate-50 border-none rounded-2xl py-4 px-6 text-lg font-bold text-slate-900 focus:ring-4 focus:ring-primary-500/10 transition-all appearance-none cursor-pointer">
                        <option value="user" {{ old('role') == 'user' ? 'selected' : '' }}>Standard Traveler (User)</option>
                        <option value="admin" {{ old('role') == 'admin' ? 'selected' : '' }}>Platform Strategist (Admin)</option>
                    </select>
                    <x-input-error :messages="$errors->get('role')" />
                </div>
                
                <div class="flex items-center pt-6 px-4">
                    <p class="text-xs text-slate-400 leading-relaxed italic">Permissions will be applied immediately upon creation of this entity in the global register.</p>
                </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    <label for="password" class="block text-xs font-black uppercase tracking-widest text-slate-400">Security Sequence</label>
                ...
                <div class="space-y-2">
                    <label for="password_confirmation" class="block text-xs font-black uppercase tracking-widest text-slate-400">Confirm Sequence</label>
            ...
            <div class="pt-6 flex justify-end gap-4 border-t border-slate-50">
                <a href="{{ route('admin.users.index') }}" class="px-8 py-4 text-slate-400 font-black uppercase tracking-widest text-xs hover:text-slate-600 transition-colors">Recall Directive</a>
