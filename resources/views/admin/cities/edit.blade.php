...
<div class="max-w-4xl">
    <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 p-10 border border-white fade-in">
        <form action="{{ route('admin.cities.update', $city->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PATCH')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="space-y-2">
                    ...
                    <select name="trip_type" id="trip_type" required class="w-full bg-slate-50 border-none rounded-2xl py-4 px-6 text-lg font-bold text-slate-900 focus:ring-4 focus:ring-primary-500/10 transition-all appearance-none cursor-pointer">
                        <option value="adventure" {{ old('trip_type', $city->trip_type) == 'adventure' ? 'selected' : '' }}>🏔️ Adventure</option>
                        <option value="culture" {{ old('trip_type', $city->trip_type) == 'culture' ? 'selected' : '' }}>🏛️ Culture</option>
                        <option value="beach" {{ old('trip_type', $city->trip_type) == 'beach' ? 'selected' : '' }}>🏖️ Beach</option>
                        <option value="romantic" {{ old('trip_type', $city->trip_type) == 'romantic' ? 'selected' : '' }}>💍 Romantic</option>
                        <option value="nature" {{ old('trip_type', $city->trip_type) == 'nature' ? 'selected' : '' }}>🌿 Nature</option>
                        <option value="shopping" {{ old('trip_type', $city->trip_type) == 'shopping' ? 'selected' : '' }}>🛍️ Shopping</option>
                    </select>
            ...
            <div class="space-y-3">
                ...
                @include('admin.partials.image-upload', ['currentImage' => $city->image])
            </div>
