...
<div class="max-w-5xl">
    <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 p-10 border border-white fade-in">
        <form action="{{ route('admin.hotels.store') }}" method="POST" class="space-y-8">
            @csrf
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    <label for="name" class="block text-xs font-black uppercase tracking-widest text-slate-400">Property Name</label>
                    <input type="text" name="name" id="name" required value="{{ old('name') }}" placeholder="e.g. Ritz Paris"
                ...
                <div class="space-y-2">
                    ...
                    <select name="city_id" id="city_id" required class="w-full bg-slate-50 border-none rounded-2xl py-4 px-6 text-lg font-bold text-slate-900 focus:ring-4 focus:ring-primary-500/10 transition-all appearance-none cursor-pointer">
                        <option value="" disabled selected>Identify Hub...</option>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div class="space-y-2">
                    ...
                    <select name="type" id="type" required class="w-full bg-slate-50 border-none rounded-2xl py-4 px-6 text-lg font-bold text-slate-900 focus:ring-4 focus:ring-primary-500/10 transition-all appearance-none cursor-pointer">
                        ...
                        <option value="mid_range" {{ old('type') == 'mid_range' ? 'selected' : '' }}>Mid Range</option>
                        <option value="luxury" {{ old('type') == 'luxury' ? 'selected' : '' }}>Luxury Elite</option>
                    </select>
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" step="0.01" name="price_per_night" id="price_per_night" required value="{{ old('price_per_night') }}" placeholder="99.99"
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" id="description" rows="3" placeholder="Describe the guest experience..."
            ...
            <div class="pt-6 flex justify-end gap-4 border-t border-slate-50">
                <a href="{{ route('admin.hotels.index') }}" class="px-8 py-4 text-slate-400 font-black uppercase tracking-widest text-xs hover:text-slate-600 transition-colors">Abort Registry</a>
